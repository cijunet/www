...
(function () {
  ...
  function fetchIndex() {
    ...
    _idx = fetch(R + 'data/pieces.msgpack.enc')
      .then(function (r) { if (!r.ok) throw new Error('no enc'); return r.arrayBuffer(); })
      .then(function (buf) {
        if (typeof msgpack === 'undefined') throw new Error('no decoder');
        var data = msgpack.decode(cjuDecrypt(new Uint8Array(buf)));
  ...
  function renderCard(p, R) {
    ...
    return '<article class="q" data-tier="' + t + '">'
      ...
      + '<div class="q-act"><button class="btn-copy" data-copy="' + esc(p.t) + '" aria-label="复制这句">复制</button>'
...
/* ── 增强：暗色模式 · 反馈入口（追加，不影响既有逻辑） ── */
(function enhance() {
  var doc = document;

  /* 暗色模式（localStorage 优先，其次系统偏好） */
  var KEY = 'ciju.theme';
...
(function round2() {
  ...
  function fetchIndexCached() {
    ...
    fetchIndexCached._p = fetch(R + 'data/pieces.msgpack.enc')
      .then(function (r) { if (!r.ok) throw new Error('no enc'); return r.arrayBuffer(); })
      .then(function (buf) {
        var data = msgpack.decode(cjuDecrypt(new Uint8Array(buf)));
        if (!window.__CJN) {
