...
export async function getManifest() {
  ...
  if (!_base) _base = baseHref();   // 兜底：今日板块等不依赖搜索 Worker 的路径也能正确取数（跨页面深度）
...
// 期望校验值：统一 gz（br 格式浏览器 DecompressionStream 不支持，已不再产出）
function expectOf(desc, ext) { return desc.hgz; }
