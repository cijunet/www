...
// 正常情况静默（info），仅当指标异常（CLS>0.25 / INP>200ms）才 console.warn，绝不抛错阻断页面。
export function mountPerf() {
  if (!('PerformanceObserver' in window)) return;
  // 最大内容绘制（LCP）：衡量首屏主体何时就绪
  try {
    new PerformanceObserver(list => {
      const e = list.getEntries().slice(-1)[0];
      if (e) console.info('[perf] LCP', Math.round(e.startTime), 'ms', e.element ? (e.element.className || e.element.tagName) : '');
